...
@{$s_str_hash{'007  '}} = (['([^\$]+)','\#1']);
@{$s_str_hash{'0247 '}} = (['(\$\$a[^\$]+)','\#1']);
@{$s_str_hash{'337  '}} = (['(\$\$aComputermedien\$\$b[^\$]+\$\$2[^\$]+)','\#1']);
@{$s_str_hash{'338  '}} = (['(\$\$aOnline-Ressource\$\$b[^\$]+\$\$2[^\$]+)','\#1']);
@{$s_str_hash{'909##'}} = (['(\$\$b[^\$]+)','\#1']);
@{$s_str_hash{'909I '}} = (['(\$\$ahybrid-e[^\$]+)','\#1']);
# @{$s_str_hash{'907  '}} = (['(\$\$g[^\$]+)','\#1']);
# @{$s_str_hash{'949  '}} = (['(\$\$s[^\$]+)','\#1']);
@{$s_str_hash{'856##'}} = (['\$\$z([^\$]+)','\#1']);
...
my $sql = "select z00_no_lines, z00_data from dsv01.z00 where z00_doc_number = ?";
...
sub check_z00 {
  ...
  @sfx_e = u_get_by_find_any(\@z00_rval, ['\$\$zonlineviaerara']);
  @z00_rval = u_get_by_rkey(\%w_data, '856..');
  @sfx_n = u_get_by_find_any(\@z00_rval, ['\$\$zOnline via e-rara']);
...
sub u_count_ex {
  my $sqlex = "select substr(z103_rec_key_1,6,9), substr(z30_sub_library,1,3), count(z30_rec_key) from dsv51.z30, dsv01.z103
