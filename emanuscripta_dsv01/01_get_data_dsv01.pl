...
#              Anpassung fuer e-manuscripta Daten in DSV01 (28.09.2018/bmt)
#              Angabe von set in der Abfrage (23.08.2019/bmt)
#---------------------------------------------------------------------------------
# 
# WICHTIG !!!!!!!!!!!
...
#      $doi = 'errordoi';
    ...
    print WRITE "$sys_no 856   L \$\$uhttp://dx.doi.org/$doi\$\$zDigitalisat in e-manuscripta\n";
...
# Sofern das seq-File nicht leer ist, wird es an das Script 02_check_in_z00_data_dsv01.pl uebergeben, welches sicherstellt, dass
# das seq-File keine Felder enthaelt, die es im Aleph in der entsprechenden Titelaufnahme bereits gibt:
if(!(-z $ud_file)) {
  system('perl '. $FindBin::Bin .'/02_check_in_z00_data_dsv01.pl '.$ud_file); 
}
...
sub usage {
    my $prog=basename($0);
    print<<EOD;
$prog: Generiere Ladelisten in ALEPH-Sequential aus e-manuscripta-OAI
